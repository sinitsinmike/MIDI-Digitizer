// File: Digitizer_midi_auto_calibration_multidispla_V5.ino
//
// Nano (DIN MIDI @31250), touch→MIDI, multi-display, RU/EN toggle (compile-time),
// EEPROM one-time calibration, CC/Notes modes, scales, arp, chord, CC pairs/labels.
...
// ── НОРМАЛИЗАЦИЯ МАКРОСА ЯЗЫКА ДЛЯ РАНТАЙМ-ВЫРАЖЕНИЙ ─
#ifdef UI_LANG_RU
  #undef UI_LANG_RU
  #define UI_LANG_RU 1
#else
  #define UI_LANG_RU 0
#endif
// теперь можно безопасно писать: (UI_LANG_RU ? "Рус" : "Eng")
...
static const uint8_t PIN_XM = 5;
static const uint8_t PIN_YM = 6;
...
static const uint8_t BTN_CAL = 7;
static const uint8_t BTN_ARP = 4;
...
#else
// 1602
void valueStr(char* buf,UiField f){} const char* labelOf(UiField f){ return ""; }
static void noteName(uint8_t note,char* out,size_t n){ static const char* nm[12]={"C","C#","D","D#","E","F","F#","G","G#","A","A#","B"}; uint8_t pc=note%12; int8_t oct=(int8_t)note/12 - 1; snprintf(out,n,"%s%d",nm[pc],oct); }
static inline void lcdClr(){ lcd.clear(); } static inline void lcdNoCursorBlink(){ lcd.noCursor(); lcd.noBlink(); }
