// File: Digitizer_MIDI_FeatToggles_PROFILES_FIX_FINAL5_MENU_FIX.ino
...
// разумный лонг-пресс (0.6 с), чтобы не путать с обычным кликом
...
//============================= PROFILES =======================================
  ...
  static void perfFromPersist(Perf& P){
    ...
    P.noteMin=G.noteMin; P.noteMax=G.noteMax; P.scaleIdx=G.scaleIdx;
  #if FEAT_TEMPO
...
static void renderStatus(uint8_t ccxLive,uint8_t ccyLive,bool noteOn,uint8_t note,uint8_t vel){
  ...
  if (G.notesMode){
    ...
    uiPrint(0,5, L("Держи ENC 0.6c","Hold ENC 0.6s"));
  }
...
static void renderMenu(){
  ...
  uiPrint(0,7, L("SAVE=D10:выход","SAVE=D10:exit"));
  uiFlush();
}
...
void loop(){
  ...
  if (btnRising(bEnc)){
    ...
    if (dt>LONG_PRESS_MS && !encRotDuringHold){
      // Тоггл меню длинным нажатием
      if (st==RUN){ st=MENU; renderMenu(); }
      else if (st==MENU){ savePersist(); st=RUN; uiClear(); renderStatus(0,0,false,0,0); } // выход с сохранением
    } else if (st==MENU){
      // клик в меню
  ...
  // --- Тоггл меню кнопкой SAVE (D10): RUN->MENU; MENU->сохранить и выйти ---
  if (btnFalling(bSave)){
    if (st==RUN){ st=MENU; renderMenu(); }
    else if (st==MENU){ savePersist(); st=RUN; uiClear(); renderStatus(0,0,false,0,0); }
  }

  // ================= MENU / LABEL_EDIT exclusive mode =================
  // Всё меню обрабатываем и выходим из loop(), чтобы ничего не затиралось RUN-логикой
  if (st==MENU){
    ...
    return; // <<< критично: выходим, чтобы не рисовать RUN-экран
  }
#if FEAT_LABELS
  if (st==LABEL_EDIT){
    int16_t d = takeEnc();
    ...
    if (btnFalling(bNext)){ labelPos=(uint8_t)((labelPos+1)%8); }
    return;
  }
#endif
