// File: src/touch_to_midi_nano_multidisplay_ru_sysex.ino
//
// Touch→MIDI (Arduino Nano, DIN-MIDI). Мульти-дисплей + RU-режим 1602 + MIDI SysEx Identity Reply.
//
...
// ── ЯЗЫК ДЛЯ 1602 (КОРОТКИЕ) ────────────────────────────────────────────────
// Включи для русских надписей (требуется модуль с RU-ROM). Иначе — EN.
//#define LCD_LANG_RU
...
// ── Опциональные LED-пункты меню (по умолчанию скрыты) ──────────────────────
...
static const uint8_t BTN_CAL = 7;  // hold+touch = Chord; коротко/долго = Cal/InvertY
static const uint8_t BTN_ARP = 4;  // hold+touch = Arp

// ── MIDI (DIN: TX=D1, 31250) ────────────────────────────────────────────────
MIDI_CREATE_INSTANCE(HardwareSerial, Serial, DINMIDI);
static const uint8_t  MIDI_CH = MIDI_CHANNEL;    // why: единая точка смены канала
...
// ── Макрос RU/EN для 1602 ───────────────────────────────────────────────────
#if defined(DISP_LCD1602) && defined(LCD_LANG_RU)
  #define L(re, en) F(re)
#else
  #define L(re, en) F(en)
#endif
...
static const __FlashStringHelper* ccNameShortWrap(uint8_t cc){ return ccNameShort(cc); }
static void ccShort(uint8_t cc,char* out,size_t n){ copyFlashStr(ccNameShortWrap(cc),out,n); }
static void valueStr(char* buf,UiField f){
  switch(f){
    case F_MODE:  snprintf(buf,16,"%s", S.notesMode?"NOTES":"CC"); break;
    ...
    case F_CCPAIR_SAVE:  snprintf(buf,16,"Press"); break;
  #if LED_MENU_ENABLED
    case F_LED_ENABLE: snprintf(buf,16,"%s", S.ledEnable?"On":"Off"); break;
    ...
    case F_TEMPO: snprintf(buf,16,"%u BPM",(unsigned)S.tempoBPM); break;
    case F_CLKIN: snprintf(buf,16,"%s",S.clockIn?"On":"Off"); break;
    ...
    case F_INVY:  snprintf(buf,16,"%s",invertY?"Yes":"No"); break;
    case F_CALNOW:{ bool ok=validSpan(G.calXmin,G.calXmax)&&validSpan(G.calYmin,G.calYmax); snprintf(buf,16,"%s",ok?"Stored":"Required"); } break;
    case F_PERFSLOT: snprintf(buf,16,"#%u",(unsigned)G.lastPerf); break;
    case F_PERFLOAD: snprintf(buf,16,"Load #%u",(unsigned)G.lastPerf); break;
    case F_PERFSAVE: snprintf(buf,16,"Save #%u",(unsigned)G.lastPerf); break;
    default: snprintf(buf,16,""); break;
  }
}
static const char* labelOf(UiField f){
  switch(f){
    case F_MODE:return "Mode"; case F_CCX:return "CC X"; case F_CCY:return "CC Y";
    case F_CCPAIRSEL:return "CC Pair"; case F_CCPAIR_X:return "Pair X"; case F_CCPAIR_Y:return "Pair Y";
    case F_CCPAIR_LABEL:return "Pair Label"; case F_CCPAIR_SAVE:return "Save CC Pairs";
  #if LED_MENU_ENABLED
    case F_LED_ENABLE:return "LED Enable"; case F_LED_BRIGHT:return "LED Bright";
  #endif
    case F_NMIN:return "NoteMin"; case F_NMAX:return "NoteMax"; case F_SCALE:return "Scale";
    case F_TEMPO:return "Tempo"; case F_CLKIN:return "Clock In";
    case F_ARPSTYLE:return "Arp Style"; case F_DIV:return "Division"; case F_CHLEN:return "Chord Len";
    case F_INVY:return "InvertY"; case F_CALNOW:return "Calibrate";
    case F_PERFSLOT:return "Perf Slot"; case F_PERFLOAD:return "Load Perf"; case F_PERFSAVE:return "Save Perf";
    default:return "";
  }
}
...
static void renderUIOLED(){
  ...
  u8g2.firstPage(); do{
    ...
    u8g2.setCursor(0,10); u8g2.print(F("MENU  (hold ENC 5s = Save&Exit)"));
    ...
    snprintf(buf,sizeof(buf),"Cal:%s  D7:Cal  D10:Save", ok?"OK":"NEEDED");
    u8g2.setCursor(0,64); u8g2.print(buf);
  }while(u8g2.nextPage());
}
static uint32_t lastStatusTsOLED=0;
static void renderStatusOLED(){
  uint32_t now=millis(); if(now-lastStatusTsOLED<100) return; lastStatusTsOLED=now;
  u8g2.firstPage(); do{
    u8g2.setFont(OLED_FONT_SMALL);
    u8g2.setCursor(0,10); u8g2.print(S.notesMode?F("MIDI Notes Mode"):F("MIDI CC Mode"));
    if (S.notesMode){
      char lo[8],hi[8],range[24]; noteName(S.noteMin,lo,sizeof(lo)); noteName(S.noteMax,hi,sizeof(hi));
      snprintf(range,sizeof(range),"Range: %s-%s",lo,hi); u8g2.setCursor(0,22); u8g2.print(range);
    } else {
      if (Labels[S.ccPairIdx][0]){ char line[28]; snprintf(line,sizeof(line),"Pair #%u %s",(unsigned)S.ccPairIdx,Labels[S.ccPairIdx]); u8g2.setCursor(0,22); u8g2.print(line); }
      else { char xnm[10],ynm[10],line[28]; copyFlashStr(ccNameShort(CCPairs[S.ccPairIdx][0]),xnm,sizeof(xnm)); copyFlashStr(ccNameShort(CCPairs[S.ccPairIdx][1]),ynm,sizeof(ynm)); snprintf(line,sizeof(line),"Pair #%u  X:%s Y:%s",(unsigned)S.ccPairIdx,xnm,ynm); u8g2.setCursor(0,22); u8g2.print(line); }
    }
    char big[24];
    if(!S.notesMode) snprintf(big,sizeof(big),"CC%u / CC%u",liveCC_X,liveCC_Y);
    else { if(liveNoteOn){ char nm[8]; noteName(liveNote,nm,sizeof(nm)); snprintf(big,sizeof(big),"%s / %u",nm,liveVel);} else snprintf(big,sizeof(big),"-- / --"); }
    u8g2.setFont(OLED_FONT_BIG); uint8_t w=u8g2.getStrWidth(big); int x=(128-w)/2; if(x<0)x=0; u8g2.setCursor(x,50); u8g2.print(big);
    u8g2.setFont(OLED_FONT_SMALL); u8g2.setCursor(0,64); u8g2.print(F("Hold ENC 5s: Menu/Save | D7:Chord | D4:Arp"));
  }while(u8g2.nextPage());
}
static void renderCalOLED(uint32_t ms_left){
  u8g2.firstPage(); do{
    u8g2.setFont(OLED_FONT_SMALL);
    u8g2.setCursor(0,12); u8g2.print(F("CALIBRATION 10s"));
    u8g2.setCursor(0,24); u8g2.print(F("Move: LB -> RB"));
    u8g2.setCursor(0,36); u8g2.print(F("       RT -> LT"));
    u8g2.setCursor(0,48); u8g2.print(F("Don't lift finger"));
    u8g2.setCursor(0,60); u8g2.print(F("Left: ")); u8g2.print(ms_left/1000); u8g2.print(F("s"));
  }while(u8g2.nextPage());
}
...
static void renderLabelEditorOLED(){
  u8g2.firstPage(); do{
    u8g2.setFont(OLED_FONT_SMALL); u8g2.setCursor(0,10); u8g2.print(F("Edit Label (pair #")); u8g2.print(S.ccPairIdx); u8g2.print(F(")"));
    ...
    u8g2.setFont(OLED_FONT_SMALL); u8g2.setCursor(0,62); u8g2.print(F("ENC:char  D9:->  D10:<-  long ENC:Save"));
  }while(u8g2.nextPage());
}
...
static void renderUI1602(){
  ...
  char v[17];
  switch(cursor){
    case F_MODE:  snprintf(v,sizeof(v), S.notesMode? (String(L("НОТЫ","NOTES")).c_str()) : (String(L("CC","CC")).c_str()) ); break;
    case F_CCX:   snprintf(v,sizeof(v),"CCX=%u",S.ccX); break;
    case F_CCY:   snprintf(v,sizeof(v),"CCY=%u",S.ccY); break;
    case F_CCPAIRSEL: snprintf(v,sizeof(v), (String(L("Пара#","Pair#"))+String(S.ccPairIdx)).c_str()); break;
    ...
    case F_CCPAIR_SAVE: snprintf(v,sizeof(v), (String(L("Нажать","Press")).c_str()) ); break;
#if LED_MENU_ENABLED
    case F_LED_ENABLE: snprintf(v,sizeof(v), (S.ledEnable? String(L("Вкл","On")):String(L("Выкл","Off"))).c_str()); break;
    ...
    case F_CLKIN: snprintf(v,sizeof(v), (S.clockIn? String(L("Вкл","On")):String(L("Выкл","Off"))).c_str()); break;
    ...
    case F_CHLEN: snprintf(v,sizeof(v),"%u",S.chordLen); break;
    case F_INVY:  snprintf(v,sizeof(v), (invertY? String(L("Да","Yes")):String(L("Нет","No"))).c_str()); break;
    case F_CALNOW:{ bool ok=validSpan(G.calXmin,G.calXmax)&&validSpan(G.calYmin,G.calYmax); snprintf(v,sizeof(v), ok? (String(L("Сохранено","Stored")).c_str()) : (String(L("Нужна","Required")).c_str()) ); } break;
    case F_PERFSLOT: snprintf(v,sizeof(v),"#%u",(unsigned)G.lastPerf); break;
    case F_PERFLOAD: snprintf(v,sizeof(v), (String(L("Проф#","Load#"))+String(G.lastPerf)).c_str()); break;
    case F_PERFSAVE: snprintf(v,sizeof(v), (String(L("Проф#","Save#"))+String(G.lastPerf)).c_str()); break;
    default: snprintf(v,sizeof(v),""); break;
  }
  lcd.setCursor(0,1); lcd.print(v);
}

static uint32_t lastStatusTs1602=0;
static void renderStatus1602(){
  uint32_t now=millis(); if(now-lastStatusTs1602<120) return; lastStatusTs1602=now;
  lcdClr(); lcdNoCursorBlink();
  if (S.notesMode){
    char lo[8],hi[8],range[17]; noteName(S.noteMin,lo,sizeof(lo)); noteName(S.noteMax,hi,sizeof(hi));
    // Строка 1
    lcd.setCursor(0,0);
...
static void renderCal1602(uint32_t ms_left){
  ...
  if (LCD_COLS>=16) snprintf(b,sizeof(b), (String(L("Ост:%lus","Left:%lus")).c_str()), (unsigned long)(ms_left/1000));
  else snprintf(b,sizeof(b), "%lus", (unsigned long)(ms_left/1000));
  lcd.print(b);
}
// 1602 Label Editor: мигающий курсор под активным символом
static void renderLabelEditor1602(){
  ...
  uint8_t col =  (LCD_LANG_RU ? 7 : 4) + labelCursor; // "Метка:"=6+1space ; "Lbl:"=4
...
// ── Квантизация/аккорды/арп (как прежде) ────────────────────────────────────
...
// why: отвечает на "Identity Request" для хостов/сканеров MIDI.
static void sendIdentityReply(){
  // Данные БЕЗ F0/F7: 7E <devID> 06 02 <mfr> <family LSB> <family MSB> <model LSB> <model MSB> <ver0..3>
  const uint8_t reply[] = {
    0x7E, MIDI_DEVICE_ID, 0x06, 0x02,
    MIDI_MFR_ID, MIDI_FAMILY_LSB, MIDI_FAMILY_MSB,
    MIDI_MODEL_LSB, MIDI_MODEL_MSB,
    MIDI_VER_0, MIDI_VER_1, MIDI_VER_2, MIDI_VER_3
  };
  DINMIDI.sendSysEx(sizeof(reply), reply, false);
}
void handleSysEx(byte* data, unsigned size){
  // Ожидаем: 7E <id|7F> 06 01   (Identity Request), без F0/F7.
  if (size >= 5 && data[0]==0x7E && data[2]==0x06 && data[3]==0x01){
    uint8_t dst = data[1];
    if (dst==0x7F || dst==MIDI_DEVICE_ID) sendIdentityReply();
  }
}
...
void setup(){
...
#if defined(DISP_LCD1602)
  lcd.init(); lcd.backlight(); lcdNoCursorBlink();
#else
  ...
  DINMIDI.setHandleSystemExclusive(handleSysEx); // SysEx handler
  startupBeep();
...
void loop(){
  ...
  if (d){
    ...
    if (labelEditActive){
      ...
      switch(cursor){
        ...
        case F_CCPAIR_LABEL: /* открывается по нажатию */ break;
#if LED_MENU_ENABLED
  ...
  // BTN_CAL: chord-hold при касании; иначе short=Cal / long=InvertY
  if (btnFalling(bCal)) calPressTs=millis();
  if (btnRising(bCal)){
    bool touchingNow = liveNoteOn; uint32_t dt=millis()-calPressTs;
    if (!touchingNow){
      if (dt>800){ invertY=!invertY; S.invY=invertY; savePerf(G.lastPerf,S); if(menuActive) RENDER_UI(); else RENDER_STATUS(); }
      else { beginCalWait(); }
    }
  }
