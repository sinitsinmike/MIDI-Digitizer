// File: Digitizer_MIDI_FeatToggles_PROFILES_FIX_FINAL.ino
// Board: Arduino Nano (ATmega328P). DIN MIDI: Serial@31250. Debug: SoftwareSerial D11.
...
#define PROF_INC_CCPAIRS  0   // include ccPairs into Perf (RAM heavy)
#define FEAT_LED          0
#define FEAT_SYSEX        0
#define FEAT_DEBUG        1   // SoftwareSerial debug TX=D11
...
//============================= UI BASIC =======================================
  ...
  static void uiBegin(){
    ...
    u8g2.begin();
    #if UI_LANG_RU
      u8g2.setFont(u8g2_font_6x12_t_cyrillic); // крупнее шрифт — читаемость
    #else
...
#endif
    char l1[20]; snprintf(l1,sizeof(l1), UI_LANG_RU? "Пара#%u %s":"Pair#%u %s",(unsigned)G.lastPair,name); uiPrintDyn(0,1,l1);
    char b[20]; snprintf(b,sizeof(b), "CC%3u/CC%3u", ccxLive, ccyLive); uiPrintDyn(0,3,b);
...
void setup(){
  ...
  loadPersist();
#if FEAT_PROFILES
  (void)profLoad(G.lastProfile); // auto-load last used profile on boot
#endif
...
void loop(){
  ...
  if (d){
    ...
    if (st==MENU){
      ...
      if (G.notesMode){
        ...
        int v=(int)G.lastPair + (d>0?+1:-1); if(v<0)v=9; if(v>9)v=0; G.lastPair=(uint8_t)v; // live pair scroll
      }
    }
  }
  ...
  if (!G.notesMode){
    ...
    if (arpHold && touch){
      ...
      arpActive=true;
  #if FEAT_CLOCK_IN && FEAT_ARP_DIV
      static uint8_t clkStepsDue=0; // shadow for local
      if (G.clockIn){ while(clkStepsDue){ clkStepsDue--; arpStep(vel);} }
      else
  #endif
