// File: Digitizer_midi_auto_calibration_multidispla_V3.ino
//
// Touch→MIDI for Arduino Nano (DIN MIDI on TX). Multi-display (SSD1306/SSD1309/SH1106/1602),
// single RU/EN compile-time toggle, one-time EEPROM calibration, CC/Notes modes,
// scales, arp, chord, 10 CC-pairs with labels, SysEx Identity Reply.
//
// ───────────────────── FIX PROLOG (SOLVES Arduino prototype issues) ─────────────────────
#include <stdint.h>

// Types known before IDE autoproto
struct DebBtn { uint8_t pin; uint8_t st; uint32_t t; };

// UI fields (renamed F_DIV -> F_DIVISION_UI)
enum UiField : uint8_t {
  F_MODE, F_CCX, F_CCY,
  F_CCPAIRSEL, F_CCPAIR_X, F_CCPAIR_Y, F_CCPAIR_LABEL, F_CCPAIR_SAVE,
  F_LED_ENABLE, F_LED_BRIGHT,     // показываются только если LED_MENU_ENABLED=1
  F_NMIN, F_NMAX, F_SCALE, F_TEMPO, F_CLKIN,
  F_ARPSTYLE, F_DIVISION_UI, F_CHLEN, F_INVY, F_CALNOW,
  F_PERFSLOT, F_PERFLOAD, F_PERFSAVE,
  F__COUNT
};

// Explicit prototypes so IDE doesn't inject broken ones
bool btnFalling(DebBtn& b);
bool btnRising (DebBtn& b);
bool btnIsDown (DebBtn& b);
void valueStr(char* buf, UiField f);
const char* labelOf(UiField f);
// ─────────────────────────────────────────────────────────────────────────────

// ── DISPLAY SELECT (pick one) ────────────────────────────────────────────────
//#define DISP_SSD1306
//#define DISP_SSD1309
//#define DISP_SH1106
//#define DISP_LCD1602
...
//#define UI_LANG_RU   // uncomment for Russian UI
...
// ── Optional LED menu items (hidden by default) ─────────────────────────────
...
// ── MIDI identity/channel (DIN on Serial@31250) ─────────────────────────────
#ifndef MIDI_CHANNEL
  #define MIDI_CHANNEL 1
#endif
#ifndef MIDI_DEVICE_ID
  #define MIDI_DEVICE_ID 0x10
#endif
#ifndef MIDI_MFR_ID
...
bool btnFalling(DebBtn& b){ uint8_t n=digitalRead(b.pin); if(n!=b.st && millis()-b.t>30){ b.st=n; b.t=millis(); return n==LOW; } return false; }
bool btnRising (DebBtn& b){ uint8_t n=digitalRead(b.pin); if(n!=b.st && millis()-b.t>30){ uint8_t was=b.st; b.st=n; b.t=millis(); return (was==LOW && n==HIGH);} return false; }
bool btnIsDown (DebBtn& b){ return b.st==LOW; }
static uint32_t encPressTs=0, calPressTs=0;
static bool encRotDuringHold=false;

// ── OLED render helpers / 1602 variants ─────────────────────────────────────
#if !defined(DISP_LCD1602)
// OLED
...
void valueStr(char* buf,UiField f){
  switch(f){
    ...
    case F_DIVISION_UI: snprintf(buf,16,"%s",DIVS[S.divIndex].label); break;
...
const char* labelOf(UiField f){
  switch(f){
    ...
    case F_DIVISION_UI:return UI_LANG_RU? "Деление":"Division";
...
#else
// 1602
void valueStr(char* buf,UiField f){} // not used on 1602
const char* labelOf(UiField f){ return ""; }
...
static void renderUI1602(){
  ...
  const __FlashStringHelper* name =
    ...
    (cursor==F_DIVISION_UI)? L("Деление","Division"):
  ...
  switch(cursor){
    ...
    case F_DIVISION_UI: snprintf(v,sizeof(v),"%s",DIVS[S.divIndex].label); break;
...
static void renderStatus1602(){
  ...
  if (S.notesMode){
    ...
    lcd.setCursor(0,0); lcd.print(L("НОТЫ ","NOTES ")); lcd.print(L("Диап:","R:")); char r[16]; snprintf(r,sizeof(r),"%s-%s",lo,hi); lcd.print(r);
    lcd.setCursor(0,1);
...
static void renderCal1602(uint32_t ms_left){
  ...
  lcd.setCursor(0,1); char b[17]; snprintf(b,sizeof(b), L("Ост:%lus","Left:%lus"), (unsigned long)(ms_left/1000)); lcd.print(b);
}
static bool labelEditActive=false; static uint8_t labelCursor=0;
static const char CHARSET[]=" ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-_/+#&:.,";
static int charsetIndex(char c){ const char* p=strchr(CHARSET,c?c:' '); return p? (int)(p-CHARSET) : 0; }
...
static void renderLabelEditor1602(){
  ...
  uint8_t col =  (UI_LANG_RU ? 7 : 4) + labelCursor;
...
#else
  u8g2.setI2CAddress(OLED_I2C_ADDR<<1); u8g2.begin(); if (OLED_USE_UTF8) u8g2.enableUTF8Print();
#endif
...
void loop(){
  ...
  if (btnRising(bEnc)){
    ...
    if (labelEditActive){
      if (dt>1500){
#if defined(DISP_LCD1602)
        pairsSave(); labelEditActive=false; RENDER_UI();
#else
        pairsSave(); labelEditActive=false; RENDER_UI();
#endif
      }
  ...
  if (d){
    ...
    if (labelEditActive){
      Labels[S.ccPairIdx][labelCursor]=charsetStep(Labels[S.ccPairIdx][labelCursor], (d>0)?+1:-1);
      RENDER_LABEL_EDITOR();
    } else if (menuActive){
      ...
      switch(cursor){
        ...
        case F_CCPAIR_LABEL: /* handled by press */ break;
      #if LED_MENU_ENABLED
        ...
        case F_DIVISION_UI:{ int v=(int)S.divIndex+(d>0?+1:-1); if(v<0)v=DIV__COUNT-1; if(v>=DIV__COUNT)v=0; S.divIndex=(uint8_t)v; } break;
